...
### BUG-034: test-selfhealing — вызов Write-Check с 3-м позиционным аргументом в ветке catch (minor, QA-приёмка self-healing 2026-09-21)
- **Симптом (латентный):** `tests\test-selfhealing.ps1:308` — `Write-Check 'harness' $false ('unhandled exception: ...')` передаёт 3-й позиционный аргумент в функцию с двумя параметрами (`:17-26`). При срабатывании catch вместо аккуратного отчёта об ошибке возникнет ParameterBindingException («A positional parameter cannot be found…»), счётчик FAIL не инкрементируется корректно.
- **Влияние:** только путь ошибки харнесса; при зелёном прогоне не выполняется (98/98 PASS подтверждён 2026-09-21).
- **Решение:** не блокирует приёмку. Фикс: добавить `[string]$Detail = ''` третьим параметром в Write-Check или конкатенировать деталь в $Label.

### BUG-035: snapshot-backoff — паттерны 'snapshot'/'exclude' слишком широкие (minor, QA-приёмка self-healing 2026-09-21)
...
- **Решение:** принято как осознанный trade-off (широкий лов transient-ошибок opencode предпочтительнее пропуска). Рекомендация: при росте ложных срабатываний сузить до составных паттернов (`FileSystem\.writeFile.*snapshot`, `failed to get diff`).
