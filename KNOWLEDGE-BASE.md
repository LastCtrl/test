...
### BUG-040: go/internal/loop — flake `TempDir RemoveAll cleanup: ... The directory is not empty` на Windows-машине с Kaspersky (minor, qa-engineer приёмка CI Go 2026-09-22) [KNOWN/ENV]
- **Симптом:** `go test ./... -count=1` в `go/` — пакет `internal/loop` падает ТОЛЬКО на cleanup (`testing.go:1617 TempDir RemoveAll cleanup: unlinkat %TEMP%\<Test>...\001\.memory\{evidence|archive}: The directory is not empty`). Ассерты зелёные, остальные 10 пакетов — ok. Раз за разом разные тесты: 22.09 зафиксировано 3 FAIL / 4 прогона пакета (TestPassDeadLettersAfterRetry, TestPassHonoursMax ×2, директории evidence и archive), соло-прогон #2 — `ok exit 0`.
- **Причина:** гонка удаления TempDir на Windows: AV (Kaspersky) держит handle на свежезаписанный JSON, RemoveAll не может удалить непустой каталог. Код не виноват: evidence пишется синхронно (`loop.go:668 bus.AppendEvidence`), heartbeat-горутинa корректно останавливается до возврата из Run (`loop.go:704-734`, `<-stopped`) — утечки горутин, пишущей в TempDir, нет (проверено grep/чтением).
- **Workaround:** повторный прогон (`go test ./internal/loop -count=1`); на GitHub-раннерах (windows-latest без корпоративного АВ) ожидаемо зелёно. Фикс по желанию: в тестах loop не полагаться на t.TempDir для дерева с частыми записями либо retry RemoveAll.
- **Статус:** KNOWN (environmental, pre-existing; впервые задокументирован в приёмке M5 21.09, повторён на этой машине 2/2 полных прогона). Дефектом приёмки CI Go (commit 81ccd83) НЕ является.
