...
| 16.13 | Рабочие окна: активен пн-пт 08:00–17:00 (окончательное решение — завтра с пользователем). Вне окна поведение — на уточнении | — |
...
**ОТКРЫТЫЕ ВОПРОСЫ (обсуждение с пользователем — завтра):**
1. Навигация «сессия → детали»: список+команда vs inline-кнопки vs ответ номером (пользователь не понял варианты — объяснить на живом примере).
2. Рабочие часы: строго пн-пт 08–17 vs бот отвечает всегда.
3. Суть ТЗ агента в деталях сессии: первая строка ТЗ после санитайзера vs только статусы (legal-конфликт).
4. Русские фразы-синонимы: подтвердить двойной ввод (фразы + /команды).
