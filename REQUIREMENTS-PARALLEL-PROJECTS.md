...
| # | Критерий | Проверяемость |
|---|----------|---------------|
| 16.1 | `/sessions` — список открытых сессий opencode: директория (agent-hq/pong-advanced/…), суть задачи (последний user-запрос), агент, время последней активности | Бот отвечает списком в течение 5 сек |
| 16.2 | `/agents` — free/busy → проект → задача (из agent-registry) | Соответствует registry на момент вызова |
...
| 16.6 | Push-уведомления: blocker `PRIORITY: critical`, dead-letter, REJECT от qa-engineer, «задача завершена» — с дедупом между тиками | Пользователь получает уведомление ≤ 2 мин после события, без дублей |
| 16.7 | Whitelist: отвечает ТОЛЬКО chat_id пользователя; чужие — игнор + лог | Сообщение с чужого chat_id не вызывает ответа |
| 16.8 | Токен бота не попадает в git (config.local.json в .gitignore + env) | `git log -p` не содержит токена |
| 16.9 | MVP ничего не пишет в шину/очереди/сессии — только читает | Код-ревью подтверждает отсутствие write-путей |
...
|------|-----|-----|----------------|
| A | dev-1 | Каркас: aiogram + прокси, whitelist, `/start`, команды-заглушки, `--once` режим | 30 мин |
| B | dev-2 | collectors.py: читатели (SQLite readonly, registry, шина, очереди, buffer) + форматтеры | 30 мин |
| C | dev-1 | Push-детектор: дедуп между тиками, состояние в `bridge-state.json` | 20 мин |
| D | qa-engineer | Unit-тесты на моках (токен не нужен) + прогон | 20 мин |
| E | code-reviewer + security-auditor | Ревью: whitelist, инъекции, read-only, токен в git | 15+15 мин |
| F | tech-writer | README: установка, настройка, деплой-инструкция | 10 мин |

**Blocker для деплоя (не для разработки):** bot-token от @BotFather (пользователь скинет позже; разработка и тесты — на моках).
